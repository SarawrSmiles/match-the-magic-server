...
def fetch_colors():
    ...
    cur.execute(
        """SELECT Colors.name, Colors.hex, Characters.name, CharacterColors.hex, Colors.complement, Garments.type, Garments.svg
...
def colors_helper():
    cur = mysql.connection.cursor()
    cur.execute(
        """SELECT CharacterColors.id, Characters.name, Colors.name, CharacterColors.hex, CharacterColors.article, CharacterColors.importance, CharacterColors.outfit_name
    ...
    for result in results:
        ...
        html_object = (
            ...
            + color_name
            + "</span> <span style='min-width: 20px; border-bottom: 1px solid black; margin: 0 5px;'> "
            + article
            + "</span> <span class='importance'> "
            + importance
            + "</span></li>"
        )
...
def fetch_movies():
    cur = mysql.connection.cursor()
    cur.execute(
        """SELECT Movies.title FROM Movies WHERE sarah_green_light = 1 ORDER BY year"""
    )
    results = cur.fetchall()
    movies = []
    for result in results:
...
def assignBestFriendship():
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            """INSERT INTO BestFriends (friend_id_1, friend_id_2) VALUES (
...
def assignMovieToCharacter():
    if request.method == "POST":
        ...
        if len(results) == 0:
            cur.execute(
                """INSERT INTO Characters (name) VALUES (%s)""", [character_name]
            )
            mysql.connection.commit()
            cur.execute("""SELECT id FROM Characters WHERE name=%s""", [character_name])
            results = cur.fetchall()
        ...
        cur.execute(
            """INSERT INTO MovieCharacters (movie_id, character_id) VALUES (%s, %s)""",
...
def addCharacterByColor():
    if request.method == "POST":
        ...
        cur.execute(
            """INSERT INTO CharacterColors (character_id, color_id, hex, outfit_name) VALUES (%s, %s, %s, %s)""",
            [character_id, color_id, character_hex, character_outfit],
        )
        mysql.connection.commit()
...
def fetch_character(name):
    ...
    character_id = results[0][0]
    cur.execute(
        """SELECT Colors.name, CharacterColors.hex, CharacterColors.importance, CharacterColors.article, CharacterColors.outfit_name
    ...
    for result in results:
        colors.append({"name": result[0], "hex": result[1]})
    ...
    cur.execute(
        """SELECT Movies.title
                   ...
                   AND MovieCharacters.character_id =%s ORDER BY Movies.year""",
        [character_id],
    )
    ...
    all_characters = []
    for result in results:
        all_characters.append({"name": str(result[0]), "id": int(result[1])})
    ...
    while len(next_character) == 0:
        next_character = [
            ...
            if character["id"] == next_character_id
        ]
